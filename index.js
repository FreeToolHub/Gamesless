...
// Audio Context (Optional simple web audio sound effects)
...
class Obstacle {
    constructor() {
        ...
        this.x = width;
        // 50% chance to spawn at top or bottom
        this.y = Math.random() > 0.5 ? height - this.height : 0;
        // 25% chance of being a fake obstacle
...
// Manage Obstacles & Collisions
function handleObstacles() {
    // Spawn rate scales slightly with speed to maintain gap consistency
...
// Random Focus Mode Event
function triggerFocusMode() {
    if (!isFocusModeActive && Math.random() < 0.002) { // Low chance every frame
        isFocusModeActive = true;
        focusOverlay.style.opacity = '1';
        
        // Remove focus mode after 3-5 seconds
        setTimeout(() => {
...
function gameLoop() {
    ...
    // Score Tracking & Dynamic Difficulty
    frames++;
    if (frames % 10 === 0) {
        score++;
        scoreEl.innerText = `Score: ${score}`;
        
        // Increase speed slightly every 150 score points
        if (score % 150 === 0) {
            gameSpeed += 0.5;
        }
    }
...
// Input Listeners for Mobile and Desktop
window.addEventListener('mousedown', (e) => {
    // Prevent triggering jump when clicking UI buttons
    if (e.target.tagName !== 'BUTTON' && isPlaying) player.jump();
});

window.addEventListener('touchstart', (e) => {
    if (e.target.tagName !== 'BUTTON' && isPlaying) {
        e.preventDefault(); // Prevents double-tap zoom or scrolling
        player.jump();
    }
}, { passive: false });
